...
// Helper: check if "submittedAt" is at least N days old
...
// Replaces all occurrences of {{key}} with data[key]
...
export async function GET() {
  try {
    // Load the callback email configuration
    const emailConfig = getEmailConfig().sevenDayCallbackEmail;
    // Use scheduleDays from config if available; otherwise, default to 7
    const scheduleDays = emailConfig.scheduleDays || 7;

    // 1. Fetch "completed" apps where sevenDayCallbackEmailSent is not true
    const query = `
      *[_type == "application" && status == "completed" && sevenDayCallbackEmailSent != true]{
        _id,
        step2,
    ...
    // 2. Filter out only those applications that are at least N days old
    const eligibleApps = applications.filter(app =>
      isAtLeastNDaysOld(app.submittedAt, scheduleDays)
    );

    if (eligibleApps.length === 0) {
      return NextResponse.json({ message: `No ${scheduleDays}-day old completed applications found for callback email.` });
    }
    ...
    // 4. Process each eligible application and send the callback email
    for (const app of eligibleApps) {
      const applicantEmail = app.step2?.email || app.account?.email;
      if (!applicantEmail) {
        console.warn(`No applicant email for doc ID: ${app._id}`);
      ...
      // For callback emails, use the applicant's first name for personalization
      const applicantFirstName = app.step2?.firstName || app.account?.firstName;

      // Populate the email template with dynamic data
      const emailHtml = populateTemplate(emailConfig.html, { firstName: applicantFirstName });
      const subject = populateTemplate(emailConfig.subject, {});
      ...
      // Mark sevenDayCallbackEmailSent as true for this application
      await client.patch(app._id).set({ sevenDayCallbackEmailSent: true }).commit();
    }

    return NextResponse.json({
      message: `callback emails sent to ${eligibleApps.length} application(s).`,
    });
  } catch (error) {
    console.error('Error sending  callback emails:', error);
    return NextResponse.json({ error: 'Error sending callback emails.' }, { status: 500 });
  }
}
